...
> 本文覆盖 Snow App 的持久化域、SQLite schema 生命周期、容错恢复、配置原子写、图像库目录迁移，以及数据管理页提供的在线快照、配置包和恢复流程。
...
当前 `PRAGMA user_version = 31`。新增表应进入当前 schema；新增列/索引或兼容旧结构的转换进入迁移；每个迁移必须幂等，完成后提升 `user_version`。不要手工编辑数据库文件或直接修改 `user_version` 来跳过迁移。
...
设置页的“数据管理”入口位于设置侧栏顶部，提供三个标签页：

- **导入与导出**：生成 `.snow-config` ZIP 包。默认排除 API Key、MCP env/headers、代理密码、SSH 路径和系统凭据；勾选敏感配置时必须设置密码，包使用 Argon2id 派生密钥和 AES-256-GCM 加密。
- **备份与恢复**：Rust 使用 SQLite Online Backup API 同时复制 `snowapp.db` 和可选的 `archive.db`，对副本运行 `quick_check`，再以 manifest、SHA-256 和原子 rename 提交 `.snowbackup`。支持 6/12 小时、每天、每周调度和 3–100 个快照保留。
- **云同步**：WebDAV 配置同步或完整数据库镜像。远端对象先加密，`state.json` 使用 revision + ETag/二次读取保护；完整镜像拉取后写入待恢复 staging，并在重启前应用。

恢复不会覆盖运行中的 SQLite 连接：后端先校验 manifest、hash 和 `quick_check`，写入 `userData/data-management/pending-restore.json`，然后请求重启。下一次启动在 storage 初始化前校验 staging、保留恢复前数据库、清理 WAL/SHM 并替换文件；初始化失败时使用 rollback 副本回退。备份目录变更后，待恢复任务仍按 pending 文件中记录的已验证目录继续处理。

完整镜像是高级模式，会影响本机会话和归档库；配置同步适合多设备日常使用。当前版本不把 checkpoint、upload、图片库、插件运行时目录或附件放入 `.snowbackup`，也不跨设备恢复 SSH、浏览器密码、Cookie、系统密钥或绝对工作区路径。附件同步和远端历史对象清理仍属于后续扩展。

加密包格式的 KDF 参数是公开 manifest/envelope 元数据；密码、派生密钥和 WebDAV Authorization header 只在主进程内使用。WebDAV 默认拒绝 HTTP，只有用户显式打开高风险选项才允许不加密连接；无 ETag 的服务器会在界面标记弱冲突保护。
...
如果不使用数据管理页，离线备份仍应遵循本节流程；运行时不要临时拼接主文件、WAL 和 SHM。在线快照的实现锚点是 `native/src/storage/services/data_management.rs`。
